...
public class SysexDataTransmissionParser
{
    public static List<UpdateMessageSpec> ConvertSysexToParameterUpdates(byte[] sysexMsg, Integra7Domain? i7)
    {
        ...
        foreach (var s in sysexMsgList)
        {
            ...
            if (!Integra7SysexHelpers.CheckIsDataSetMsg(s)) continue;

            var payload = Integra7SysexHelpers.ExtractPayload(s);
            if (payload.Length < 4)
            {
                // Too short to even hold an address. ExtractPayload already logged why the payload
                // looks the way it does; this is just the "and so we can't parse it" consequence, so
                // a note (not a warning) is enough here.
                Log.Debug("Data-set payload too short to hold an address ({Length} byte(s)); nothing to parse.",
                    payload.Length);
                continue;
            }
            ...
            while (currentLocation < payload.Length)
            {
                ...
                if (p is null)
                {
                    // An address this build does not know. Looping again would not advance -- address
                    // and currentLocation would both stay put -- so stop here rather than spin, and
                    // keep whatever parameters were already understood.
                    Log.Warning(
                ...
                {
                    // A truncated message declaring more parameter bytes than it actually carries.
                    // Slicing would read past the end of the payload.
                    Log.Warning(
                ...
                SysexParameterValueInterpreter.Interpret(parResult, p.ParSpec, out var rawVal, out var displayValue);
