...
**Tech stack:** .NET 10, C# 13, Avalonia 12, ReactiveUI, NUnit 4.
...
public class MorphedToneTests
{
    ...
    public void A_parameter_governed_by_a_discriminator_comes_from_the_winner()
    {
        ...
        // A real variant path, not "MFX Parameter 1" -- that one is a commented-out definition in the
        // database and LookupIndex answers -1 for it, so a test written against it would slip through the
        // unknown-path branch and pin nothing. A capture holds only the variants valid under its own MFX
        // Type, which is also why both corners here share a type.
